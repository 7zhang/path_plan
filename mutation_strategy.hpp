...
public:
	 ...
	 */
	typedef boost::tuple< individual_ptr, DVectorPtr > mutation_info;
...
//	virtual mutation_info operator()( const population& pop, individual_ptr bestIt, size_t i ) = 0;
	virtual mutation_info operator()( const population& pop, population &pop2, individual_ptr bestIt, size_t i ) = 0;
	/**
...
//		individual_ptr tmpInd( boost::make_shared< individual >( *pop[ i ]->vars() ) );
		...
		*tmpInd = *pop[i];
		...
		// j = ++j % varCount();
		// for (; k != j; j = ++j % varCount())
		do	
		{
			if (k == j || genrand() < crossover()) {
				(*tmpInd->vars())[ j ] = (*pop[ urn[ 0 ] ]->vars() )[ j ] +
					weight() * ( (*pop[ urn[ 1 ] ]->vars() )[ j ] - (*pop[ urn[ 2 ] ]->vars())[ j ] );
			}
			j = ++j % varCount();
...
//		origin = pop[ urn[ 0 ] ]->vars();
		...
		return mutation_info( tmpInd, pop[ urn[ 0 ] ]->vars() );
	}
