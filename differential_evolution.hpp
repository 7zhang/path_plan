...
public:
	 ...
	 */
	differential_evolution( size_t varCount, size_t popSize, typename processors< T >::processors_ptr processors, constraints_ptr constraints, bool minimize, termination_strategy_ptr terminationStrategy, selection_strategy_ptr selectionStrategy,
				mutation_strategy_ptr mutationStrategy, de::listener_ptr listener )
	try
		: m_varCount( varCount ), m_popSize( popSize ), m_pop1( boost::make_shared< population >( popSize, varCount, constraints ) ), m_pop2( boost::make_shared< population >( popSize, varCount ) ), m_bestInd( m_pop1->best( minimize ) ),
	...
	void run()
	{
		try
		{
			...
			for( size_t genCount = 0 ; m_terminationStrategy->event( m_bestInd, genCount ); ++genCount ) 
			{
				...
				{
					mutation_strategy::mutation_info mutationInfo( ( *m_mutationStrategy)( *m_pop1, *m_pop2, bestIndIteration, i ) );
